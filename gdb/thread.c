		     ...
		     struct thread_info *thr)
{
  ...
  if (requested_threads != NULL && *requested_threads != '\0')
    ...
    return itset_width_contains_thread (current_itset, ITSET_WIDTH_ALL,
					thr);
}

void
print_one_thread_info (struct ui_out *uiout, struct thread_info *tp)
{
  struct cleanup *chain2, *old_chain;
  int core;
  ...
  if (ui_out_is_mi_like_p (uiout))
...

    {
      ...
      ui_out_field_int (uiout, "inferior-id", get_thread_inferior (tp)->num);
      ui_out_field_int (uiout, "per-inferior-id", tp->num_inf);
    }
...
print_thread_info (struct ui_out *uiout, char *requested_threads, int pid)
{
  ...
  ALL_INFERIORS (inf)
      ...
      for (tp = thread_list; tp; tp = tp->next)
	{
	  ...
	  print_one_thread_info (uiout, tp);
	}
