  ...
  if (inferior_ptid != null_ptid && target_has_execution ())
    {
	 ...
	 restart it.  */
      ...
      if (from_tty
          && !query (
            _ ("The program being debugged has been started already.\n\
...
get_return_value (struct symbol *func_symbol, struct value *function)
{
  ...
  switch (
    gdbarch_return_value (gdbarch, function, value_type, NULL, NULL, NULL))
    {
...
{
  if (rv->type == NULL || check_typedef (rv->type)->code () == TYPE_CODE_VOID)
    return;
  ...
  if (target_can_async_p ())
    {
      ...
      if (args != NULL && startswith (args, "-a"))
        all_threads = 1;
...
Show whether `finish' prints the return value."),
                           NULL, NULL, show_print_finish, &setprintlist,
                           &showprintlist);
}
