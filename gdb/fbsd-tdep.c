...
fbsd_make_corefile_notes (struct gdbarch *gdbarch, bfd *obfd, int *note_size)
{
  ...
  if (note_desc && !note_desc->empty ())
    {
      note_data.reset (
        elfcore_write_note (obfd, note_data.release (), note_size, "FreeBSD",
                            NT_FREEBSD_PROCSTAT_AUXV, note_desc->data (),
                            note_desc->size ()));
      if (!note_data)
        return NULL;
    }
  ...
  if (note_desc && !note_desc->empty ())
    {
      note_data.reset (
        elfcore_write_note (obfd, note_data.release (), note_size, "FreeBSD",
                            NT_FREEBSD_PROCSTAT_VMMAP, note_desc->data (),
                            note_desc->size ()));
      if (!note_data)
        return NULL;
    }
  ...
  if (note_desc && !note_desc->empty ())
    {
      note_data.reset (
        elfcore_write_note (obfd, note_data.release (), note_size, "FreeBSD",
                            NT_FREEBSD_PROCSTAT_PSSTRINGS, note_desc->data (),
                            note_desc->size ()));
      if (!note_data)
        return NULL;
    }
...
fbsd_core_info_proc_status (struct gdbarch *gdbarch)
{
  ...
  for (int i = 0; i < ngroups; i++)
    gdb_printf ("%s ",
                pulongest (
                  bfd_get_32 (core_bfd, descdata + kp->ki_groups + i * 4)));
  gdb_printf ("\n");
  value
  ...
  gdb_printf ("Data size: %s pages\n",
              pulongest (
                bfd_get (addr_bit, core_bfd, descdata + kp->ki_dsize)));
  gdb_printf ("Stack size: %s pages\n",
              pulongest (
                bfd_get (addr_bit, core_bfd, descdata + kp->ki_ssize)));
  gdb_printf ("Text size: %s pages\n",
              pulongest (
                bfd_get (addr_bit, core_bfd, descdata + kp->ki_tsize)));
  gdb_printf ("Resident set size: %s pages\n",
              pulongest (
                bfd_get (addr_bit, core_bfd, descdata + kp->ki_rssize)));
