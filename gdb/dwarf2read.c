                          ...
                          sect_offset sect_off, ULONGEST length)
{
  ...
  dwarf2_per_cu_data *the_cu
    = obstack_new<dwarf2_per_cu_data> (&objfile->objfile_obstack);
   ...
   offset_type elements)
{
  ...
  for (offset_type i = 0; i < elements; i += 3)
	...
	= obstack_new<signatured_type> (&objfile->objfile_obstack);
			      ...
			      rcuh_kind section_kind)
{
  ...
  while (info_ptr < end_ptr)
      ...
      else
	{
	  ...
	  dwo_tu = NULL;
	  sig_type = obstack_new<signatured_type> (&objfile->objfile_obstack);
...
create_type_unit_group (struct dwarf2_cu *cu, sect_offset line_offset_struct)
{
  ...
  type_unit_group *tu_group
    = obstack_new<type_unit_group> (&objfile->objfile_obstack);
			      ...
			      unsigned int is_dwz)
{
  ...
  while (info_ptr < section->buffer + section->size)
	...
	this_cu = obstack_new<dwarf2_per_cu_data> (&objfile->objfile_obstack);
      else
	{
	  signatured_type *sig_type
	    = obstack_new<signatured_type> (&objfile->objfile_obstack);
