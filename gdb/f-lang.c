  ...
  if (!is_all_contiguous && (repack_array_slices || is_string_p))
    {
	 ...
	 array value DEST.  */
      ...
      if (value_lazy (array)
        ...
        {
          fortran_array_walker<fortran_lazy_array_repacker_impl>
            p (array_slice_type, value_address (array) + total_offset, dest);
          p.walk ();
        }
      else
        {
          fortran_array_walker<fortran_array_repacker_impl>
            p (array_slice_type, value_address (array) + total_offset,
