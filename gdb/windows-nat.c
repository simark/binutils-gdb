      ...
      if (have_saved_context)
        {
	     ...
	     "saved_context.  */
          memcpy (&th->context, &saved_context, __COPY_CONTEXT_SIZE);
...
windows_per_inferior::handle_unload_dll ()
{
  ...
  auto iter
    = std::remove_if (windows_process.solibs.begin (),
                      windows_process.solibs.end (), [&] (windows_solib &lib) {
                        if (lib.load_addr == lpBaseOfDll)
                          {
                            DEBUG_EVENTS ("Unloading dll \"%s\".",
                                          lib.name.c_str ());
                            return true;
                          }
                        return false;
  ...
  struct target_waitstatus *ourstatus)
{
  ...
  gdb::unique_xmalloc_ptr<char> s
    = (target_read_string ((CORE_ADDR) (uintptr_t)
  ...
  if (windows_process.wow64_process)
    {
	 ...
	 can't be correctly handled by gdb.  */
      ...
      if (windows_process.wow64_dbgbreak != nullptr)
        {
          HANDLE thread = CreateRemoteThread (windows_process.handle, NULL, 0,
