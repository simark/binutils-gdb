    ...
    if (strcmp ("register", bt_ctf_get_decl_event_name (list[i])) == 0)
      {
        ...
        gdb_assert (
          0
          == strcmp ("contents", bt_ctf_get_decl_field_name (field_list[0])));
...
#define SET_ARRAY_FIELD(EVENT, SCOPE, VAR, NUM, ARRAY)                       \
  do                                                                         \
    {                                                                        \
      ...
      const struct bt_definition *def;                                       \
                                                                             \
      lu32 = (uint32_t) bt_ctf_get_uint64 (                                  \
        bt_ctf_get_field ((EVENT), (SCOPE), #NUM));                          \
  ...
  while (1)
    {
      ...
      scope = bt_ctf_get_top_level_scope (event, BT_EVENT_FIELDS);
      int32 = (int32_t) bt_ctf_get_int64 (
        bt_ctf_get_field (event, scope, "number"));
