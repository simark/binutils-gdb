...
solib_aix_bfd_open (const char *pathname)
{
  ...
  gdb_bfd_ref_ptr archive_bfd (
    solib_bfd_fopen (found_pathname.get (), found_file));
  if (archive_bfd == NULL)
    {
...
_initialize_solib_aix ()
{
  ...
  add_setshow_boolean_cmd ("aix-solib", class_maintenance, &solib_aix_debug,
                           _ ("\
Control the debugging traces for the solib-aix module."),
                           _ ("\
Show whether solib-aix debugging traces are enabled."),
                           _ ("\
When on, solib-aix debugging traces are enabled."),
                           NULL, show_solib_aix_debug, &setdebuglist,
                           &showdebuglist);
}
