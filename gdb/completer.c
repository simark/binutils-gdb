...
completion_tracker::discard_completions ()
{
  ...
  m_entries_hash.reset (
    htab_create_alloc (INITIAL_COMPLETION_HTAB_SIZE, entry_hash_func,
                       entry_eq_func, htab_delete_entry<completion_hash_entry>,
                       xcalloc, xfree));
}
                                             ...
                                             int end)
{
  ...
  if (m_lowest_common_denominator_unique)
  ...
  else
    {
      ...
      struct list_builder
      {
        list_builder (char **ml) : match_list (ml), index (1)
        { /* Nothing.  */
        }
