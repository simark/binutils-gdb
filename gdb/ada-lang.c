  ...
  if (saw_non_ascii)
    {
      ...
      try
        {
          convert_between_encodings (host_charset (),
                                     is_utf8 ? HOST_UTF32 : ada_source_charset,
                                     (const gdb_byte *)
                                       encoding_buffer.c_str (),
                                     encoding_buffer.length (), 1, &storage,
                                     translit_none);
        }
  ...
  if (elt_type->code () == TYPE_CODE_ARRAY)
    {
      ...
      type->set_length (
        ((our_len * elt_bitsize + HOST_CHAR_BIT - 1) / HOST_CHAR_BIT));
    }
...
struct match_data
{
  explicit match_data (std::vector<struct block_symbol> *rp) : resultp (rp) {}
  ...
  if (sym == NULL)
    {
      if (!found_sym && arg_sym != NULL)
        add_defn_to_vec (*resultp, fixup_symbol_section (arg_sym, objfile),
                         block);
      found_sym = false;
      arg_sym = NULL;
    }
  else
    {
      if (sym->aclass () == LOC_UNRESOLVED)
        ...
        {
          found_sym = true;
          add_defn_to_vec (*resultp, fixup_symbol_section (sym, objfile),
                           block);
        }
    }
  return true;
                       ...
                       struct objfile *objfile)
{
          ...
          if (sym->aclass () != LOC_UNRESOLVED)
            {
              if (sym->is_argument ())
                ...
                {
                  found_sym = true;
                  add_defn_to_vec (result, fixup_symbol_section (sym, objfile),
                                   block);
                }
            }
        }
  ...
  if (!found_sym && arg_sym != NULL)
    {
      add_defn_to_vec (result, fixup_symbol_section (arg_sym, objfile), block);
    }
  ...
  if (!lookup_name.ada ().wild_match_p ())
    {
	 ...
	 They aren't parameters, right?  */
      if (!found_sym && arg_sym != NULL)
        {
          add_defn_to_vec (result, fixup_symbol_section (arg_sym, objfile),
                           block);
        }
    }
}
        ...
        if (check_tag && address != 0 && ada_is_tagged_type (static_type, 0))
          {
            ...
            if (real_type != NULL)
              return to_fixed_record_type (real_type, NULL,
                                           value_address (
...
ada_is_character_type (struct type *type)
{
  ...
  return (
    name != NULL
    && (type->code () == TYPE_CODE_INT || type->code () == TYPE_CODE_RANGE)
    && (strcmp (name, "character") == 0 || strcmp (name, "wide_character") == 0
                              ...
                              LONGEST high, operation_up &op)
{
  ...
  if (ada_is_direct_array_type (value_type (lhs)))
    index = longest_to_int (
      value_as_long (m_val->evaluate (nullptr, exp, EVAL_NORMAL)));
  else
    {
                      ...
                      value *arg3)
{
  ...
  return value_from_longest (type, (value_less (arg1, arg3)
  ...
  if (noside == EVAL_AVOID_SIDE_EFFECTS)
  ...
  else if (discrete_type_p (type_arg))
    {
      ...
      switch (op)
        ...
        case OP_ATR_LAST:
          return value_from_longest (range_type, ada_discrete_type_high_bound (
                                                   range_type));
        case OP_ATR_LENGTH:
                         ...
                         struct type *context_type)
{
  if (resolve (exp, deprocedure_p, parse_completion, tracker, context_type))
    return (
      make_operation<ada_funcall_operation> (std::move (owner),
       ...
       Otherwise, assume that the target type is an int.  */
    {
      if (expect_type != NULL)
        return ada_value_ind (
          value_cast (lookup_pointer_type (expect_type), arg1));
      else
                                 ...
                                 struct expression *exp, enum noside noside)
{
  ...
  switch (type->code ())
    ...
    case TYPE_CODE_STRUCT:
      {
        ...
        if (noside == EVAL_AVOID_SIDE_EFFECTS)
          return value_zero (ada_aligned_type (type), lval_memory);
        return unwrap_value (
          ada_value_subscript (callee, nargs, argvec.data ()));
      }
    case TYPE_CODE_ARRAY:
      if (noside == EVAL_AVOID_SIDE_EFFECTS)
        {
      ...
      return unwrap_value (
        ada_value_subscript (ada_coerce_to_simple_array (callee), nargs,
                             argvec.data ()));
    case TYPE_CODE_PTR: /* Pointer to array */
      ...
      return unwrap_value (
        ada_value_ptr_subscript (callee, nargs, argvec.data ()));
                                 ...
                                 int disabled, int from_tty)
{
  ...
  std::unique_ptr<ada_catchpoint> c (
    new ada_catchpoint (gdbarch, ex_kind, sal, addr_string.c_str (), tempflag,
                        disabled, from_tty));
...
{
public:
  ada_language () : language_defn (language_ada)
  { /* Nothing.  */
  }
                           ...
                           struct language_arch_info *lai) const override
  {
    ...
    add (
      arch_integer_type (gdbarch, gdbarch_int_bit (gdbarch), 0, "positive"));
    add (builtin->builtin_void);

    struct type *system_addr_ptr = lookup_pointer_type (
