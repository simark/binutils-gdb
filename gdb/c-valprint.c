...
static void
print_unpacked_pointer (CORE_ADDR addr, const gdb_byte *valaddr,
			int embedded_offset, struct ui_file *stream,
			struct type *type, struct type *unresolved_elttype,
			struct type *elttype,
			const struct value_print_options *options,
			struct gdbarch *gdbarch, int recurse)
{
  int want_space = 0;
  ...
  if (options->symbol_print)
    want_space = print_address_demangle (options, gdbarch, addr, stream,
					 demangle);
		   ...
		   const struct value_print_options *options)
{
  ...
  if (TYPE_LENGTH (type) > 0 && TYPE_LENGTH (unresolved_elttype) > 0)
	      ...
	      TARGET_CHAR_BIT * TYPE_LENGTH(type)))
	{
	  ...
	  if (options->stop_print_at_null)
	    {
		  ...
		  (temp_len < len && temp_len < options->print_max
			  ...
			  valaddr + embedded_offset + temp_len * eltlen, eltlen,
			  byte_order) != 0); ++temp_len)
		;
	  ...
	  LA_PRINT_STRING(stream, unresolved_elttype, valaddr + embedded_offset,
			  len, NULL, force_ellipses, options);
	  i = len;
	}
      else
	{
	  ...
	  val_print_array_elements (type, valaddr, embedded_offset, address,
  ...
  else
    {
      ...
      print_unpacked_pointer (addr, valaddr, embedded_offset, stream, type,
			      unresolved_elttype, elttype, options, gdbarch,
			      recurse);
    }

}

static void c_val_print_ptr (struct type *type, const gdb_byte *valaddr,
			     int embedded_offset, CORE_ADDR address,
			     struct ui_file *stream, int recurse,
			     const struct value *original_value,
			     const struct value_print_options *options)
	     ...
	     const struct value_print_options *options)
{
  ...
  switch (TYPE_CODE (type))
    ...
    case TYPE_CODE_PTR:
      c_val_print_ptr (type, valaddr, embedded_offset, address, stream, recurse,
    ...
    case TYPE_CODE_INT:
      c_val_print_int (type, unresolved_type, valaddr, embedded_offset, address,
		       stream, recurse, original_value, options);
      break;
