      ...
      if (reparse)
        {
		 ...
		 manually.  */
              ...
              target_resources_ok
                = target_can_use_hardware_watchpoint (type, i,
                                                      other_type_used);
...
{
  return (
    (gdbarch_has_global_breakpoints (target_gdbarch ()) || aspace1 == aspace2)
    && addr1 == addr2);
}
                                  ...
                                  bool is_tracepoint)
{
  if (locspec != nullptr)
    return (
      breakpoint_ops_for_location_spec_type (locspec->type (), is_tracepoint));
  return &code_breakpoint_ops;
}
...
break_range_command (const char *arg, int from_tty)
{
  ...
  std::unique_ptr<breakpoint> br (
    new ranged_breakpoint (get_current_arch (), sal_start, length,
...
clear_command (const char *arg, int from_tty)
{
  ...
  found.erase (std::unique (found.begin (), found.end (),
                            [] (const breakpoint *bp_a,
                                const breakpoint *bp_b) {
                              return compare_breakpoints (bp_a, bp_b) == 0;
                            }),
               found.end ());
  ...
  if (b->loc == NULL)
    {
	 ...
	 a condition or dprintf arguments.  */
      if (b->extra_string == NULL)
        ...
        {
          gdb_printf (_ (" (%s,%s) pending."), b->locspec->to_string (),
                      b->extra_string.get ());
        }
      else
        {
...
ordinary_breakpoint::print_it (const bpstat *bs) const
{
  ...
  if (bp_temp)
    ...
    uiout->message ("Breakpoint %pF, ", signed_field ("bkptno", number));
  ...
  for (size_t i = 0; i < lsal.sals.size (); i++)
    {
      ...
      std::unique_ptr<tracepoint> tp (
        new tracepoint (gdbarch, type_wanted, lsal.sals[i],
                        std::move (locspec), NULL, std::move (cond_string),
                        std::move (extra_string), disposition, thread, task,
                        ignore_count, from_tty, enabled, flags,
                        canonical->special_display));
  ...
  if (tp->control.single_step_breakpoints == NULL)
    {
      std::unique_ptr<breakpoint> b (
        new momentary_breakpoint (gdbarch, bp_single_step,
                                  current_program_space, null_frame_id,
                                  tp->global_num));
