      ...
      if (i == 0)
        {
          ...
          HEADER_FILES (dbxread_objfile) = (struct header_file *) xmalloc (
            10 * sizeof (struct header_file));
        }
      else
        {
          i *= 2;
                ...
                if (dependencies_used >= dependencies_allocated)
                  {
                    ...
                    dependency_list = (legacy_psymtab **) alloca (
                      (dependencies_allocated *= 2)
                      * sizeof (legacy_psymtab *));
...
read_ofile_symtab (struct objfile *objfile, legacy_psymtab *pst)
{
  ...
  pst->compunit_symtab
    = end_compunit_symtab (text_offset + text_size, SECT_OFF_TEXT (objfile));
