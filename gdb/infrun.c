...
displaced_step_finish (thread_info *event_thread, enum gdb_signal signal)
{
  ...
  /* Fixup may need to read memory/registers.  Switch to the thread
     that we're fixing up.  Also, displaced_step_instruction_executed_successfully relies on
     the current thread.  */
...
stop_all_threads (void)
{
  ...
  for (pass = 0; pass < 2; pass++, iterations++)
      ...
      while (1)
	{
	  ...
	  infrun_log_debug ("stop_all_threads %s %s",
	  ...
	  if (event.ws.kind == TARGET_WAITKIND_NO_RESUMED
	      ...
	      else
		{
		  ...
		  infrun_log_debug ("target_wait %s, saving status for "
				    "%d.%ld.%ld",
				    target_waitstatus_to_string
				      (&event.ws).c_str (),
				    t->ptid.pid (),
  ...
  for (inferior *inf : all_non_exited_inferiors (ecs->target))
    {
      ...
      if (thread == NULL)
	{
	  infrun_log_debug ("TARGET_WAITKIND_NO_RESUMED (expect process exit)");
	  prepare_to_wait (ecs);
	  return 1;
	}
    }
				    ...
				    struct frame_info *frame)
{
  ...
  infrun_log_debug ("exception resume at %s",
		    paddress (get_objfile_arch (probe->objfile), handler));
