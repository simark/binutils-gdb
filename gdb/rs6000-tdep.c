                           ...
                           struct type *type)
{
  ...
  return (
    tdep->ppc_fp0_regnum >= 0 && regnum >= tdep->ppc_fp0_regnum
    && regnum < tdep->ppc_fp0_regnum + ppc_num_fprs
    && type->code () == TYPE_CODE_FLT
    && (type->length () != builtin_type (gdbarch)->builtin_double->length ()));
}
...
rs6000_program_breakpoint_here_p (gdbarch *gdbarch, CORE_ADDR address)
{
  ...
  if (target_read_memory (address, target_mem, PPC_INSN_SIZE) == 0)
    {
      uint32_t insn
        = (uint32_t) extract_unsigned_integer (target_mem, PPC_INSN_SIZE,
