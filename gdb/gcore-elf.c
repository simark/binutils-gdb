                                     ...
                                     const char *human_name, void *cb_data)
{
  ...
  if (strcmp (sect_name, ".reg") == 0)
    data->note_data->reset (
      elfcore_write_prstatus (data->obfd, data->note_data->release (),
                              data->note_size, data->lwp,
                              gdb_signal_to_host (data->stop_signal),
                              buf.data ()));
  else
    data->note_data->reset (
      elfcore_write_register_note (data->obfd, data->note_data->release (),
  ...
  if (tdesc_xml != nullptr && *tdesc_xml != '\0')
    {
      ...
      note_data->reset (
        elfcore_write_register_note (obfd, note_data->release (), note_size,
                                     ".gdb-tdesc", tdesc_xml, tdesc_len));
    }
}
