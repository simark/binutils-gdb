...
struct xtensa_gdbarch : public gdbarch_tdep
{
  xtensa_gdbarch (unsigned int spill_size_, xtensa_register_t* regmap_)
  : spill_size (spill_size_), regmap (regmap_)
  {
  }
  ...
  unsigned int spill_location = -1;
  unsigned int spill_size = 0;

  char *unused = NULL;				/* Placeholder for compatibility.  */
  ...
  unsigned long *fp_layout = NULL; /* Layout of custom/TIE regs in 'FP' area.  */
  unsigned int fp_layout_bytes = 0; /* Size of layout information (in bytes).  */
  unsigned long *gregmap = NULL;

  /* Cached register types.  */
  struct ctype_cache *type_entries = NULL;
};
