...
make_gdb_type (struct gdbarch *gdbarch, struct tdesc_type *ttype)
{
  ...
  {
  public:
    gdb_type_creator (struct gdbarch *gdbarch) : m_gdbarch (gdbarch) {}
...
void
copy_inferior_target_desc_info (struct inferior *destinf,
                     ...
                     tdesc_unknown_register_ftype unk_reg_cb)
{
  ...
  htab_up reg_hash (
    htab_create (37, htab_hash_pointer, htab_eq_pointer, NULL));
  for (const tdesc_feature_up &feature : target_desc->features)
    for (const tdesc_reg_up &reg : feature->registers)
      {
...
{
public:
  print_c_feature (std::string &file) : print_c_tdesc (file)
  {
    /* Trim ".tmp".  */
